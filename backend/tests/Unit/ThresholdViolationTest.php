...
class ThresholdViolationTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        // Threshold default: Suhu 24-32°C, Kelembaban 80-95%
    ...
    /**
     * Suhu di atas temp_max (32°C) HARUS memicu violation 'temp_high'.
     */
    public function test_violation_when_temperature_exceeds_max(): void
    {
        ...
        $this->assertStringContainsString('32.00', $tempHigh['message']);
    }
    ...
    /**
     * Suhu di bawah temp_min (24°C) HARUS memicu violation 'temp_low'.
     */
    ...
    /**
     * Kelembaban di atas humidity_max (95%) HARUS memicu violation.
     */
    public function test_violation_when_humidity_exceeds_max(): void
    {
        $sensor = SensorData::factory()->highHumidity(97.50)->create([
            'temperature' => 28.00,
        ]);
    ...
    /**
     * Kelembaban di bawah humidity_min (80%) HARUS memicu violation.
     */
    ...
    {
        $sensor = SensorData::factory()->create([
            'temperature' => 35.00, // Di atas max 32
            'humidity' => 97.00,    // Di atas max 95
        ]);
    ...
    {
        $sensor = SensorData::factory()->create([
            'temperature' => 10.00, // Di bawah min 24
            'humidity' => 50.00,    // Di bawah min 80
        ]);
    ...
    {
        $sensor = SensorData::factory()->create([
            'temperature' => 40.00, // Di atas max 32
            'humidity' => 50.00,    // Di bawah min 80
        ]);
    ...
    /**
     * Suhu tepat di batas maksimal (32°C) TIDAK boleh memicu violation.
    ...
    {
        $sensor = SensorData::factory()->create([
            'temperature' => 32.00, // Tepat di max
            'humidity' => 85.00,
        ]);
    ...
    /**
     * Suhu tepat di batas minimal (24°C) TIDAK boleh memicu violation.
    ...
    {
        $sensor = SensorData::factory()->create([
            'temperature' => 24.00, // Tepat di min
            'humidity' => 85.00,
        ]);
    ...
    /**
     * Kelembaban tepat di batas (80% dan 95%) TIDAK boleh memicu violation.
     */
    ...
    /**
     * Suhu 32.01°C (0.01 di atas max) HARUS memicu violation.
    ...
    {
        $sensor = SensorData::factory()->create([
            'temperature' => 32.01, // Tepat sedikit di atas max
            'humidity' => 85.00,
        ]);
        ...
        $tempHigh = collect($violations)->firstWhere('type', 'temp_high');
        $this->assertNotNull($tempHigh, 'Suhu 32.01°C (di atas max 32°C) seharusnya memicu violation');
    }
}
